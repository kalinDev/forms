  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center bg-slate-50">
      <form className="min-h-screen max-w-5xl mt-5 mb-5">
        ...
        <RadioGroupDemo label={"Você se considera uma pessoa nervosa? "} values={["Não", "Sim"]}  onAnswer={(answer: Answer) => handleAnswers(answer)}  questionId="3"/>
        <RadioGroupDemo label="Em alguma época de sua vida, você consultou um médico por causa de nervosismo ou tristeza?" values={["Não", "Sim, há mais de um ano", "Sim, no último ano", "Sim, nos últimos 3 meses"]} onAnswer={(answer: Answer) => handleAnswers(answer)} questionId="4"/>
        <RadioGroupDemo label="Alguma vez, por estar se sentindo nervoso ou triste, você tomou algum chá (por exemplo, erva-cidreira, maracujá), ou então, algum preparado à base de ervas?" values={["Não", "Sim, há mais de um ano", "Sim, no último ano", "Sim, nos últimos 3 meses"]} showDescription={true} onAnswer={(answer: Answer) => handleAnswers(answer)} questionId="5"/>
        ...
        Pode ser um padre, um pastor, um médium, ou mesmo alguém que nos foi indicado por um conhecido. Você já procurou alguém assim, para melhorar do nervosismo?" values={["Não", "Sim, há mais de um ano", "Sim, no último ano", "Sim, nos últimos 3 meses"]} showDescription={true} onAnswer={(answer: Answer) => handleAnswers(answer)} questionId="6"/>
        <RadioGroupDemo label="Alguma vez você já foi internado em hospital para tratamento dos nervos?" values={["Não", "Sim, há mais de um ano", "Sim, no último ano", "Sim, nos últimos 6 meses"]} onAnswer={(answer: Answer) => handleAnswers(answer)} questionId="7"/>
